...
public class FormatterFunction {
	@FunctionMeta(description = "Format object using message pattern", example = "${a:format(payload.dateFrom,'yyyy-MM-dd')}")
	public static String format(Object obj, String pattern) {
		return Formats.forObject().format(obj, pattern);
	}

	@FunctionMeta(description = "Format number with message pattern with auto scaling", example = "${a:formatNumber(12345, '0.#', 'B')}")
	...
	@FunctionMeta(description = "Format number in percentage", example = "${a:percentage(4, 11, 2)}")
	...
	@FunctionMeta(description = "Shorten a message", example = "${a:shorten('123456789', 5)} will be '1...9'")
